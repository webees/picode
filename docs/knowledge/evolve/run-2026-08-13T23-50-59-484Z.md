...
- **D082 Skill harness 落地**（C1 skill-spec + C2 skill-wiring）：
...
- **D083 缓项**：skills-ref 官方校验工具接入（npm 包需联网，picode 无裸网 D010；自研已覆盖等价语义）
- **D084 缓项**：skill 打包/导入双轨机械实现（M6 已文档约定，机械实现需网下载器）
- **D085 拒**：skill-creator / 评价循环（依赖 LLM 评价，超出承载体系边界）
- **D086 拒**：allowed-tools 机械强制（与 09 ACL 关系未定，仅解析不强制）
...
- **C3 `task-skill-docs`（本任务）**：DECISIONS D082 落地 + D055 行移除 skills_root 死键标记 +
  D083-086 记录；decision-catalog §15 Skill harness；skill-spec.md 规范正文（frontmatter + 目录结构 +
...
2. **skill 打包/导入双轨机械实现**（D084）：托管只读 vs 可编辑副本的 CLI 下载/安装器（需网）
3. **skill-creator 评价循环**（D085）：evals/benchmark/variance，依赖 LLM 评价，独立立项
4. **allowed-tools 与 ACL 整合设计**（D086）：skill 级工具白名单与 09 tool-profiles 关系设计先行
