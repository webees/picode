...
**本文是 picode 技能承载体系（Skill Harness，D082）的 SKILL.md 规范唯一正文**：`skills/` 下每个技能目录内的 `SKILL.md` 必须按本文书写。  
...
|`allowed-tools`|可选（实验性）|非空 string[]|同上（仅解析不强制，D086 留档）|
...
`SKILL.md` 正文 = instructions 层全文，**永不被自动灌入系统 prompt**（D082-4）。加载沿三层递进，agent 自判激活：
