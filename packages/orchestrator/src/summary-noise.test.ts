...
test("D092: READY_MESSAGE_TEXT / CONTINUATION_PROMPT / CONTINUATION_SUMMARY_HEADER 非空且含关键语义", async () => {
...
test("D092: SUMMARY_STRIP_NOISE 恰好 = [READY_MESSAGE_TEXT, CONTINUATION_PROMPT]", async () => {
...
test("D092: summary-noise.ts 零 import（无任何外部依赖，可安全被任意模块引用）", async () => {
