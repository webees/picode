...
/**
 * Serve 代理（D058 / D070 / D093）：GET {base_url}/session/{serveId}/message →
 * 抽取最近一条 assistant 消息的 `info.tokens.total`。只读、有界超时（ERR-01）：
 * serve 失联 / 非 200 / 非 JSON 一律返回 `{ error }`，绝不让 HTTP 进程崩。
 *
 * D093：由 dashboard-server 上移为平台原语（orchestrator 导出），
 * supervise 与 dashboard 共用一份；dashboard-server live.ts 改为薄壳再导出。
 */
